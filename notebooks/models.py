...
class NotebookQuerySet(models.query.QuerySet):
...
class Notebook(models.Model):
    ...
    file = models.FileField(
        _("Le notebook"), upload_to="notebook/%Y/%m/%d/"
    )
    timestamp = models.DateTimeField(auto_now_add=True)
    title = models.CharField(max_length=255, null=False, unique=True,verbose_name='Titre')
    description = models.TextField(max_length=255, null=True, verbose_name='Description')
    vue = models.FileField(
        _("Une version html du notebook pour l'affichage"), upload_to="notebook/%Y/%m/%d/",blank=True
    )
...
def notify_comment(**kwargs):  # pragma: no cover
    """Handler to be fired up upon comments signal to notify the dev of a
    given Notebook."""
